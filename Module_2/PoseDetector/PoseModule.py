...
def main_poseModule():
    f = open('Module_2/PoseDetector/AnimationFile.txt', 'r+')
    f.truncate(0)
    cap = cv2.VideoCapture('data/videos/detect_pose.mp4')
    pTime = 0
    ...
    while True:
        ...
        if len(lmList) != 0:
            print(lmList[14])
            with open("Module_2/PoseDetector/AnimationFile.txt", 'a') as f:
                f.writelines(str(lmList[14])+'\n')
